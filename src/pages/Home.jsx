...
const categories = [
  { name: 'Fruit Plants', icon: '🥭', desc: 'Alphonso Mango, Kesar Mango, Guava, Pomegranate & more', color: 'from-orange-400 to-orange-600' },
  { name: 'Vegetable Plants', icon: '🥬', desc: 'Tomato, Brinjal, Cabbage, Cauliflower & Chili seedlings', color: 'from-green-400 to-green-600' },
  { name: 'Spice Plants', icon: '🌶️', desc: 'Ginger, Curry Leaf, Mirchi & aromatic spice crops', color: 'from-red-400 to-red-600' },
  { name: 'Flowering & Ornamental', icon: '🌺', desc: 'Beautiful flowering & decorative plants for gardens', color: 'from-pink-400 to-pink-600' },
  { name: 'Plantation Crops', icon: '🌴', desc: 'Coconut, Date Palm, Sugarcane & large-scale farming', color: 'from-emerald-400 to-emerald-600' },
  { name: 'Other Crops', icon: '🌾', desc: 'Khirni, Chikoo, Dragon Fruit & specialty plants', color: 'from-amber-400 to-amber-600' },
];
...
const stats = [
  { label: 'Happy Farmers', value: '10,000+', icon: '😊' },
  { label: 'Plants Delivered', value: '5,00,000+', icon: '🌱' },
  { label: 'Districts Served', value: '30+', icon: '🏙️' },
          ...
          {categories.map((cat, i) => (
            <motion.div key={cat.name} variants={item}>
              ...
              >
                ...
                <span className="text-5xl mb-3 group-hover:scale-110 transition-transform duration-300">{cat.icon}</span>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        ...
        {featured.length > 0 && (
          <div className="text-center mt-8">
            <Link to="/products" className="inline-block bg-primary text-white font-bold px-10 py-3 rounded-full hover:bg-primary-light transition-all shadow-lg hover:shadow-xl hover:scale-105">
              View All Plants →
            </Link>
          </div>
        )}
      ...
      <section className="bg-primary-dark text-white py-20 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: '🏥', title: 'Govt. Approved Nursery', desc: 'Government certified hi-tech nursery ensuring quality, authenticity and healthy plants.' },
              { icon: '🌱', title: 'Wholesale Rates', desc: 'Buy directly from the nursery at best wholesale prices. Bulk orders welcome for farmers.' },
              { icon: '🚚', title: 'Pan-Maharashtra Delivery', desc: 'We deliver across Maharashtra. Safe packaging ensuring plants reach you in perfect condition.' },
              { icon: '👨‍🌾', title: '15+ Years Experience', desc: 'Run by experienced horticulturists with deep knowledge of plant cultivation and care.' },
              { icon: '📞', title: 'Expert Guidance', desc: 'Free consultation on plant selection, planting techniques and crop management.' },
              { icon: '🔄', title: 'Healthy Plant Guarantee', desc: 'All plants are quality-checked. Replacement guaranteed for any damaged plants on delivery.' },
            ].map((f, i) => (
              <motion.div
                key={f.title}
      ...
      <section className="max-w-6xl mx-auto px-4 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((t, i) => (
            ...
            >
              <div className="text-gold text-lg mb-2">{'★'.repeat(t.rating)}{'☆'.repeat(5 - t.rating)}</div>
      ...
      <section className="bg-gradient-to-r from-primary via-primary-light to-accent py-20 px-4 text-white text-center">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} viewport={{ once: true }}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/products" className="bg-white text-primary font-bold px-10 py-4 rounded-full text-lg hover:bg-accent-light hover:text-white transition-all shadow-xl hover:scale-105">
              Shop Now
            </Link>
